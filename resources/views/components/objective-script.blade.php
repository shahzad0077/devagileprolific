...
function updateepicflag() {
    ...
    if ($('#flag_type').val() != '') {
        ...
        $('#success-flag-error').html('<div class="alert alert-danger" role="alert">Please fill out all required fields.</div>');
    }
}
...
$(addButton).click(function(){
    ...
    if(x < maxField){ 
        x++; //Increment field counter
        var fieldHTML = '<div class="d-flex w-75 mb-3">  <div class="form-group mb-0"><input type="text" placeholder="" onkeypress="return onlyNumberKey(event)"  id="customFile" class="form-control target_value"/><label for="objective-name">Quarter '+x+' Target Value</label></div><a href="javascript:void(0);"  class="remove_button btn btn-light-danger"><i class="fa fa-minus"></i></a></div>';
...
{
$('#edit-c'+id).html('<input type="text" class="form-control" id="title'+id+'" value="'+comment+'" onfocusout="updateComment('+id+')">');
}
...
{
$('#edit-val'+id).html('<input type="text" class="form-control w-50" style="font-size:12px" id="value'+id+'" value="'+val+'" onfocusout="updateQvalue('+id+')">');
}
...
function getteam()
{
    ...
    var type = "{{ $organization->type }}";  
        ...
        $('select[name="key-team"]').append('<option value="'+ course.id +'">' + course.team_title+ '</option>');
        });
        }else{
        $('#key-team').empty();
        }
