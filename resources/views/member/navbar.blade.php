    ...
    <div class="d-flex flex-column flex-shrink-0 bg-light" style="width: 100%; height: 100vh;">
        <ul class="nav nav-pills nav-flush flex-column mb-auto text-center sidebar" id="navbarSupportedContent">
            <li class="nav-item" data-toggle="tooltip" data-placement="right" data-original-title="Go Back">
                <a href="{{url('dashboard/organization/Business-Units')}}" class="nav-link" aria-current="page">
            ...
            <li data-toggle="tooltip" data-placement="right" data-original-title="Value Streams">
                <a href="#panel-new" class="nav-link">
                    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
                      <path d="M22.598 5.70514L15.7613 2.66014C14.758 2.2168 13.2413 2.2168 12.238 2.66014L5.4013 5.70514C3.67464 6.47514 3.41797 7.52514 3.41797 8.08514C3.41797 8.64514 3.67464 9.69514 5.4013 10.4651L12.238 13.5101C12.7396 13.7318 13.3696 13.8485 13.9996 13.8485C14.6296 13.8485 15.2596 13.7318 15.7613 13.5101L22.598 10.4651C24.3246 9.69514 24.5813 8.64514 24.5813 8.08514C24.5813 7.52514 24.3363 6.47514 22.598 5.70514Z" fill="#787878"/>
                      <path opacity="0.4" d="M14.0004 19.8802C13.557 19.8802 13.1137 19.7868 12.7054 19.6118L4.84203 16.1118C3.64036 15.5752 2.70703 14.1402 2.70703 12.8218C2.70703 12.3435 3.09203 11.9585 3.57036 11.9585C4.0487 11.9585 4.4337 12.3435 4.4337 12.8218C4.4337 13.4518 4.9587 14.2685 5.54203 14.5252L13.4054 18.0252C13.7787 18.1885 14.2104 18.1885 14.5837 18.0252L22.447 14.5252C23.0304 14.2685 23.5554 13.4635 23.5554 12.8218C23.5554 12.3435 23.9404 11.9585 24.4187 11.9585C24.897 11.9585 25.282 12.3435 25.282 12.8218C25.282 14.1285 24.3487 15.5752 23.147 16.1118L15.2837 19.6118C14.887 19.7868 14.4437 19.8802 14.0004 19.8802Z" fill="#787878"/>
                      <path opacity="0.4" d="M14.0004 25.6668C13.557 25.6668 13.1137 25.5734 12.7054 25.3984L4.84203 21.8985C3.54703 21.3268 2.70703 20.0318 2.70703 18.6085C2.70703 18.1301 3.09203 17.7451 3.57036 17.7451C4.0487 17.7451 4.4337 18.1301 4.4337 18.6085C4.4337 19.3435 4.86536 20.0085 5.54203 20.3118L13.4054 23.8118C13.7787 23.9751 14.2104 23.9751 14.5837 23.8118L22.447 20.3118C23.112 20.0201 23.5554 19.3435 23.5554 18.6085C23.5554 18.1301 23.9404 17.7451 24.4187 17.7451C24.897 17.7451 25.282 18.1301 25.282 18.6085C25.282 20.0318 24.442 21.3151 23.147 21.8985L15.2837 25.3984C14.887 25.5734 14.4437 25.6668 14.0004 25.6668Z" fill="#787878"/>
                    </svg>
                </a>
            </li>
            ...
            <li data-toggle="tooltip" data-placement="right" data-original-title="Backlog">
                <a href="{{url('dashboard/organization/'.$organization->slug.'/VS-Backlog')}}" @if (url()->current() == url('dashboard/organization/'.$organization->slug.'/VS-Backlog')) class="nav-link active" @else class="nav-link"  @endif>
                    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
            ...
            <li data-toggle="tooltip" data-placement="right" data-original-title="Linking">
                <a href="{{url('dashboard/linking/'.$organization->slug.'/'.$organization->type)}}" @if (url()->current() == url('dashboard/linking/'.$organization->slug.'/'.$organization->type)) class="nav-link active" @else class="nav-link"  @endif >
                    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
             ...
             <li data-toggle="tooltip" data-placement="right" data-original-title="Teams">
                <a href="{{url('dashboard/organization/'.$organization->slug.'/VS-TEAMS')}}" @if (url()->current() == url('dashboard/organization/'.$organization->slug.'/VS-TEAMS')) class="nav-link active" @else class="nav-link"  @endif>
                    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
            ...
            <li data-toggle="tooltip" data-placement="right" data-original-title="Impediments Flags">
                <a href="{{url('dashboard/flags/'.$organization->slug.'/impediments/stream')}}" @if (url()->current() == url('dashboard/flags/'.$organization->slug.'/impediments/stream')) class="nav-link active" @else class="nav-link"  @endif >
                    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
                      <path d="M25.6673 25.6665H2.33398C1.85565 25.6665 1.45898 25.2698 1.45898 24.7915C1.45898 24.3132 1.85565 23.9165 2.33398 23.9165H25.6673C26.1457 23.9165 26.5423 24.3132 26.5423 24.7915C26.5423 25.2698 26.1457 25.6665 25.6673 25.6665Z" fill="#787878"/>
                      <path d="M11.375 4.66683V25.6668H16.625V4.66683C16.625 3.3835 16.1 2.3335 14.525 2.3335H13.475C11.9 2.3335 11.375 3.3835 11.375 4.66683Z" fill="#787878"/>
                      <path opacity="0.4" d="M3.5 11.6668V25.6668H8.16667V11.6668C8.16667 10.3835 7.7 9.3335 6.3 9.3335H5.36667C3.96667 9.3335 3.5 10.3835 3.5 11.6668Z" fill="#787878"/>
